...
export default function DonationForm() {
  ...
  const confirm = async () => {
    ...
    let { isAnonymous, firstName, email, state, city, lastName, amount, frequency } =
      data;
    ...
    const response = await fetch(`${baseUrl}donations`, {
      ...
      body: JSON.stringify({
        amount,
        currency: "usd",
        frequency: frequency ? frequency : "one-off",
        firstName,
        email,
        state,
        city,
  ...
  const steps = useMemo(() => [
    ...
    },
    {
      ...
      component: <UserForm handleChange={handleChange} data={data} />,
    },
    // {
    //   id: "payment",
    //   label: "Payments",
    ...
    return (
      <div className="flex flex-col mt-11 max-w-full leading-none w-[670px] max-md:mt-10">
        <div className="flex gap-5 items-start self-start text-white whitespace-nowrap">
          {amounts?.map((amt) => (
            <div key={amt} className="flex flex-col w-[60px]">
              <button
                className={`overflow-hidden gap-2.5 self-stretch p-4 hover:bg-gray-300 rounded-lg bg-neutral-400 ${
                  amt == data.amount
        ...
        <div className="flex flex-col mt-6 w-full whitespace-nowrap max-md:max-w-full">
          <label htmlFor="customAmount" className="self-start text-black">
            Amount($)
          </label>
        ...
        <div className="flex flex-col pb-4 mt-6 w-full max-md:max-w-full">
          <label htmlFor="frequency" className="self-start text-black">
            Donation Frequency
          </label>
          <select
            id="frequency"
