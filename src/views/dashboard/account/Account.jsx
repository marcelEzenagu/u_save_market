...
// Modal component
const Modal = ({ title, children, onClose, onSave, loading, data }) => {
  return (
    <div className="fixed inset-0 bg-black w-full bg-opacity-75 flex justify-center lg:items-center z-50 animated fadeInDown">
      <div className="bg-white  w-full rounded-md p-6 max-w-[500px] animated fadeInDown">
        ...
        <div className="mt-4 flex justify-end space-x-2">
          ...
          <button
            className="px-4 py-2 bg-regal-sky-blue text-white rounded-md"
            onClick={onSave}
            disabled={loading}
          >
            {!data ? (loading ? "loading..." : "Edit") : "Save"}
          </button>
        </div>
      </div>
    </div>
...
function Account() {
  ...
  const [updateUserProfile, { isLoading: profileLoading }] =
    useUpdateUserProfileMutation();
  const [updateUserProfilePicture, { isLoading: profilePictureLoading }] =
    useUpdateUserProfilePictureMutation();
  ...
  const handleSaveProfilePicture = async () => {
    ...
    if (base64String) {
      try {
        ...
        const updateUserDetails = await updateUserProfilePicture({
        ...
        handleError(
          err,
          "update profile picture failed, Please check your image might be too large"
        );
      }
    } else {
    ...
    <div className="p-4">
      ...
      <div className="rounded-md border p-5 mt-4">
        <div className="flex flex-row items-center justify-between">
          <div className="flex flex-row items-center space-x-4">
            <div className="w-11 h-11 flex flex-col items-center justify-center rounded-full border border-regal-sky-blue">
              <img
                ...
                src={
                  user?.profilePicture ||
                  "https://as2.ftcdn.net/jpg/02/15/84/43/160_F_215844325_ttX9YiIIyeaR7Ne6EaLLjMAmy4GvPC69.jpg"
                }
                alt={user?.firstName}
              />
            </div>
          ...
          <button
            className="flex items-center py-1 px-4 gap-1 rounded-md border border-regal-sky-blue text-regal-sky-blue text-xs md:text-sm font-[500]"
      ...
      <div className="rounded-md border p-5 mt-4">
        <div className="flex flex-row items-center justify-between">
          <div className="flex flex-row items-center space-x-4">
            <p className="text-sm text-start text-regal-black font-[700] capitalize">
              Personal info
            </p>
          </div>
          <button
        ...
        <div className="min-w-[400px]">
          <div className="grid grid-cols-2 w-full">
            <div className="mt-5">
              <h6 className="text-xs  md:text-sm text-regal-light-gray font-[500]">
                First name
              </h6>
            ...
            <div className="mt-7 mb-4">
              ...
              <p className="text-xs md:text-sm text-regal-black mt-2 font-[500] flex items-center">
                <img src={countries[0]?.flag} alt="" className="w-6 rounded-sm mr-1" />{" "}
                {countries[0]?.number} 8037048205
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="min-w-[400px]">
          <div className="grid grid-cols-2 w-full">
            ...
            <div className="mt-7 mb-4">
              ...
              <p className="text-xs md:text-sm text-regal-black mt-2 font-[500]">Nguel</p>
            </div>
            <div className="mt-7 mb-4">
              <h6 className="text-xs md:text-sm text-regal-light-gray font-[500]">
                ZIP Code
          ...
          {imagePreview && (
            <div>
              <img
                src={imagePreview}
                alt=""
                className="w-32 h-32 mx-auto my-5 object-cover rounded-full text-center"
              />
            </div>
          )}
      ...
      {isEditingAddress && (
        ...
        >
          <input
            ...
            className="w-full mb-2 p-2 border"
          />
          {handleErrorMessagesList("country")}
          <input
            type="text"
            name="state"
            value={data.state}
          ...
          <input
            ...
            className="w-full mb-2 p-2 border"
          />
          {handleErrorMessagesList("zipCode")}
          <p className="text-red-600 text-xs">{errMsg && errMsg}</p>
        </Modal>
      )}
    </div>
