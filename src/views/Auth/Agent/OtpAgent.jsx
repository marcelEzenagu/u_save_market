...
import { useForgotPasswordAgentMutation, useVerifyOtpAgentMutation } from "../../../features/auth/authApiSlice";
...
const OtpAgent = ({otpType}) => {
  ...
  const [resetPasswordAgent, { isLoading: loading }] = useForgotPasswordAgentMutation();
  ...
  useEffect(()=>{
      ...
      if (!verifiedDetails?.email || !verifiedDetails?.requestID){
        navigate('agent/forgot-password');
      }
     }, [verifiedDetails])
     ...
     const handleResendOtp = async () => {
      ...
      setErrorMessagesList([]);
      try {
        const userData = await resetPasswordAgent({email : verifiedDetails?.email}).unwrap();
  ...
  return (
    <div className="block lg:flex   items-start px-4  max-w-[1366px] mx-auto">
            <div className=" hidden lg:block w-1/2 my-auto h-[94vh]">
        <img
          src={RightImage}
          alt="Right Side"
